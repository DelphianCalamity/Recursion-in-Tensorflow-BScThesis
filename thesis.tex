...
Below, we present an example of calling functions, expressed as a functional program and the way it will be converted into a dataflow graph and get executed in TensorFlow: 
 \textit{\textbf{\begin{center}result = f(4) + f(5)\\f(x) = g(x+1)\\ g(y) = y \end{center}}}

\begin{figure}
\centering
\includegraphics[scale=0.65]{figures/Tf_recursion}
...
\begin{flushleft}
\textit{Note that, the evaluation of  Return under the following condition was left undefined :}\\
$ Return(d, name), where \ $ \\
...
Contrary to the case of iteration, the execution of non-tail recursive functions imposes what might constitute a significant performance issue.
...
In the execution of iterative constructs, it is easy to specify a maximum number of overlapping iteration steps, but in most cases of recursion, the execution of one function-instance depends on the execution of another one


    \section{Distributed Execution}


here,explain what is deadness propagation, and why is needed. 

express the problem, occurred with functions, and how was solved.

addition of state machines
